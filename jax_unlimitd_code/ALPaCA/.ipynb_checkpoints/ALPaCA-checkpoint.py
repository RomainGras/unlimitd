...
class ALPaCA:
    def __init__(self, config, sess, graph=None, preprocess=None, f_nom=None):
        ...
        self.sigma_eps = self.config['data_noise']
    ...
    def construct_model(self):
        with self.graph.as_default():
            ...
            self.K = tf.Variable(tf.random.normal([last_layer, self.y_dim]), name='K_init') #\bar{K}_0

            self.L_asym = tf.Variable(tf.random.normal([last_layer, last_layer]), name='L_asym')
 # cholesky decomp of \Lambda_0
