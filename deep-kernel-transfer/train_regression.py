...
if params.method=='MAML':
    ...
    optimizer = torch.optim.Adam([{'params': bb.parameters(), 'lr': 1e-5}])
...
if params.method=='DKT':
    model = DKT(bb).cuda()
    optimizer = torch.optim.Adam([{'params': model.model.parameters(), 'lr': 0.1},
                                {'params': model.feature_extractor.parameters(), 'lr': 0.1}])
    params.checkpoint_dir += f"_{configs.kernel_type}"
    print(params.checkpoint_dir)
    
    # Initialize data for spectral kernel with real input
    if configs.kernel_type=='spectral':
        inps, labs = [], []
        for _ in range(5):
            inputs, labels = provider.get_train_batch()
        ...
        model.model.covar_module.initialize_from_data(z, labels)
    ...
    projection_matrix = proj_sketch(model.diff_net, batches, params.subspacedim).cuda()
    ...
    # Unlimitd-F training
    if params.conv_net_not_differentiated:
        model = unlimited_plus(bb, backbone.simple_net(), has_scaling_params=True, projection_matrix=projection_matrix, method=params.method).cuda()
    else:
        model = unlimited_plus(None, bb, has_scaling_params=True, projection_matrix=projection_matrix, method=params.method).cuda() 
    model.load_checkpoint(params.checkpoint_dir)
    optimizer = torch.optim.Adam([{'params': model.parameters(), 'lr': 0.001}, {'params': model.model.covar_module.scaling_params, 'lr': 0.001}])
    params.checkpoint_dir += f"_{configs.kernel_type}"
    for epoch in range(params.stop_epoch//2, params.stop_epoch):
        model.train_loop(epoch, provider, optimizer)
