...
class differentialDKTnogpy(MetaTemplate):
    def __init__(self, model_func, diff_net, n_way, n_support):
        super(differentialDKTnogpy, self).__init__(model_func, n_way, n_support)
        ## GP parameters
    ...
    def train_loop(self, epoch, train_loader, optimizer, print_freq=10):
        ...
        for i, (x,_) in enumerate(train_loader):
            ...
            # Forward pass
            phi = self.diff_net(z_train)
            
            # Dictionary to store statistics for each BatchNorm layer
            bn_stats = {}
            for name, module in self.diff_net.named_modules():
                if isinstance(module, nn.BatchNorm2d):
                    bn_stats[name] = {
                        "running_mean": module.running_mean.clone(),
                        "running_var": module.running_var.clone()
                    }
            # Freeze BatchNorm statistics for the epoch
            for name, module in self.diff_net.named_modules():
                if isinstance(module, nn.BatchNorm2d):
                    # Set to eval mode to freeze the running stats (but still allow gradient updates on gamma and beta)
                    module.eval()
                    # Restore the saved running mean and var
                    module.running_mean = bn_stats[name]["running_mean"]
                    module.running_var = bn_stats[name]["running_var"]

            
            ###############
            
            # Compute J(x1)
            jac_func = create_jac_func(self.diff_net)
            ...
            if i % print_freq==0:
                ...
                # with torch.no_grad():
                    ...
                    # Compute the NTKs
                    # ntks_ss = square_ntk(jac1)
                    # ntks_qs = support_query_ntk(jac1, jac2)
                    # ntks_qq = square_ntk(jac2)
    ...
    def optim_correct(self, x, n_ft, lr, temp):
        # optimal parameters for datasets ;
        # temp = .3 for omniglot
        # temp = .04 for CUB
        # temp = .3 for miniImagenet
    ...
    def test_loop(self, test_loader, optim_based=False, n_ft=0, lr=0, temp=.3, record=None, return_std=False):
        ...
        for i, (x,_) in enumerate(test_loader):
            ...
            if optim_based:
                correct_this, count_this, loss_value = self.optim_correct(x, n_ft, lr, temp)
            else:
