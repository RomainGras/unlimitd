...
class ModifiedCDKT(MetaTemplate):
    def __init__(self, model_func, n_way, n_support):
        super(ModifiedCDKT, self).__init__(model_func, n_way, n_support)
        self.device = 'cuda:0'
        ## GP parameters
    ...
    def train_loop(self, epoch, train_loader, optimizer, print_freq=10):
        ...
        for i, (x,_) in enumerate(train_loader):
            # print(f"x shape :{x.shape}") # x shape :torch.Size([5, 17, 3, 84, 84])
            self.n_query = x.size(1) - self.n_support
            ...
            # print(f"z shape : {z_train.shape}") # z shape : torch.Size([85, 1600])
            output = self.model(z_train)
            # print(f"output shape : {output.shape}") # output shape : torch.Size([5, 85, 85])
            ...
            if i % print_freq==0:
                ...
                print('Epoch [{:d}] [{:d}/{:d}] | Outscale {} | NEGMEAN {:f} | Loss {:f}'.format(epoch, i, len(train_loader), outputscales_str, self.NEGMEAN, loss.item()))
