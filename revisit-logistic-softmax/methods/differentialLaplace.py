...
class differentialLaplace(MetaTemplate):
    """
    ...
    def __init__(self, model_func, diff_net, n_way, n_support):
        super(differentialLaplace, self).__init__(model_func, n_way, n_support)
        self.device = 'cuda:0'
        ## GP parameters
    ...
    def correct(self, x):
        ...
        with torch.no_grad():
            ...
            support_mean_vec = self.diff_net(z_support).T
            # if(self.normalize_mean): support_mean_vec = F.normalize(support_mean_vec, p=2, dim=1)
