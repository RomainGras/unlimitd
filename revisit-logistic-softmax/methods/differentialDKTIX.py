...
class differentialDKTIX(MetaTemplate):
    ...
    def get_model_likelihood_mll(self, train_x_list=None, train_y_list=None):
        if(train_x_list is None): train_x_list=[torch.ones(100, 12794880).cuda()]*self.n_way
...
class NTKernel(gpytorch.kernels.Kernel):
    ...
    def forward(self, x1, x2, diag=False, **params):
        # omniglot
        x1 = x1.reshape(x1.size(0), 3, 28, 28)
        x2 = x2.reshape(x2.size(0), 3, 28, 28)
        ...
        # ResNet        
        # x1 = x1.reshape(x1.size(0), 3, 224, 224)
        # x2 = x2.reshape(x2.size(0), 3, 224, 224)
...
class ExactGPLayer(gpytorch.models.ExactGP):
    ...
    def __init__(self, train_x, train_y, likelihood, net, scaling_params, c=0, kernel='linear'):
        ...
        ## Linear kernel
        if(kernel=='linear'):
            self.covar_module = NTKernel(net, scaling_params, c, normalize=False)
