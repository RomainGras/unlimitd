...
class differentialCDKT(MetaTemplate):
    ...
    def train_loop(self, epoch, train_loader, optimizer, print_freq=10):
        ...
        for i, (x,_) in enumerate(train_loader):
            ...
            sub_x_train = self.feature_extractor.forward(x_train)  # First CNN or Transformer layer gives an embedding
            z_train = self.compute_jacobian(sub_x_train)
    ...
    def correct(self, x):
        ...
        with torch.no_grad():
            ...
            sub_x_support = self.feature_extractor.forward(x_support)  # First CNN or Transformer layer gives an embedding
            z_support = self.compute_jacobian(sub_x_support).detach()
            if(self.normalize): z_support = F.normalize(z_support, p=2, dim=1)
            support_outputs = self.model(z_support)
            ...
            sub_x_query = self.feature_extractor.forward(x_query)  # First CNN or Transformer layer gives an embedding
            z_query = self.compute_jacobian(sub_x_query).detach()
            if(self.normalize): z_query = F.normalize(z_query, p=2, dim=1)
